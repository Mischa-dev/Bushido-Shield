...
const DataAPI = {
  // Configuration - now defaults to the shared dashboard backend
  USE_LOCAL_STORAGE: false,
  BACKEND_URL: 'http://localhost:5179/api',
  ...
  async getState() {
    if (this.USE_LOCAL_STORAGE) {
      ...
      const state = await this._httpRequest('/state');
      return serverToStorage(state);
    }
  },
  ...
  async updateStatePartial(storageUpdates) {
    ...
    const serverPayload = storageToServer(storageUpdates);
    if (!Object.keys(serverPayload).length) return false;
...
const STORAGE_KEY_MAP = {
  ...
  'bs:logs': 'logs',
  'bs:enabled:global': 'enabledGlobal',
  'bs:activeProfile': 'activeProfile',
  'bs:lastSync': 'lastSync',
  'bs:pausedUntil': 'pausedUntil'
};

function serverToStorage(state = {}) {
  const mapped = {};
  for (const [storageKey, serverKey] of Object.entries(STORAGE_KEY_MAP)) {
    if (Object.prototype.hasOwnProperty.call(state, serverKey)) {
      mapped[storageKey] = state[serverKey];
    }
  }
  return mapped;
}

function storageToServer(updates = {}) {
  const mapped = {};
  for (const [storageKey, value] of Object.entries(updates)) {
    const serverKey = STORAGE_KEY_MAP[storageKey];
    if (serverKey) {
...
async function get(keys) {
  if (DataAPI.USE_LOCAL_STORAGE && API?.storage?.local) {
    return new Promise((resolve) => API.storage.local.get(keys, (v) => resolve(v || {})));
  }
  const requested = Array.isArray(keys) ? keys : [keys];
  const state = await DataAPI.getState();
  const result = {};
...
async function set(obj) {
  if (DataAPI.USE_LOCAL_STORAGE && API?.storage?.local) {
    return new Promise((resolve) => API.storage.local.set(obj, () => resolve(true)));
  }
  await DataAPI.updateStatePartial(obj);
  return true;
}
...
async function seedIfNeeded() {
  if (!DataAPI.USE_LOCAL_STORAGE) return;
...
function renderQuickDevicesOverview(state){
  ...
  filtered.slice(0, 20).forEach(device => {
    ...
    pauseWrapper.className = 'pause-controls';
    pauseWrapper.style.position = 'relative';
    
    if (isPaused) {
      const resumeBtn = document.createElement('button');
      resumeBtn.className = 'pause-trigger';
      const remaining = Math.max(0, new Date(device.pausedUntil).getTime() - Date.now());
      const mins = Math.ceil(remaining / 60000);
      resumeBtn.textContent = mins < 60 ? `${mins}m` : `${Math.floor(mins/60)}h`;
      resumeBtn.title = `Resume protection (${mins} min remaining)`;
      resumeBtn.addEventListener('click', async () => {
        await updateDevice(device.id, { pausedUntil: null });
        toast(`✓ ${device.name} resumed`);
        const freshState = await loadState();
        renderDevices(freshState);
        renderQuickDevices(freshState);
        renderQuickDevicesOverview(freshState);
      });
      pauseWrapper.appendChild(resumeBtn);
    } else {
      const pauseBtn = document.createElement('button');
      pauseBtn.className = 'pause-trigger';
      pauseBtn.textContent = 'Pause ▾';
      pauseBtn.title = 'Pause protection';
      
      const pauseMenu = document.createElement('div');
      pauseMenu.className = 'pause-menu';
      
      [15, 30, 60, 240, 1440].forEach(mins => {
        const btn = document.createElement('button');
        const label = mins >= 60 ? `${mins / 60}h` : `${mins}m`;
        const fullLabel = mins >= 60 ? `${mins / 60} hour${mins > 60 ? 's' : ''}` : `${mins} min`;
        ...
        btn.addEventListener('click', async () => {
          ...
          toast(`✓ ${device.name} paused for ${fullLabel}`);
      ...
      pauseBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        document.querySelectorAll('.pause-menu').forEach(m => m.style.display = 'none');
        pauseMenu.style.display = pauseMenu.style.display === 'grid' ? 'none' : 'grid';
      });
      
      pauseWrapper.append(pauseBtn, pauseMenu);
    }
    
    // Append elements to row
    row.append(dot, nameEl, profileSelect, pauseWrapper);
    quickDevicesOverview.appendChild(row);
  });
  
  // Close pause menus when clicking outside
...
async function renderDevices(state){
  ...
  list.forEach((d)=>{
    ...
    if (isPaused) {
      const remaining = Math.max(0, new Date(d.pausedUntil).getTime() - Date.now());
      const mins = Math.ceil(remaining / 60000);
      const resumeBtn = document.createElement('button');
      resumeBtn.className = 'pause-trigger';
      resumeBtn.textContent = mins < 60 ? `${mins}m` : `${Math.floor(mins/60)}h`;
      resumeBtn.title = `Resume (${mins} min remaining)`;
      resumeBtn.addEventListener('click', async () => {
        await updateDevice(d.id, { pausedUntil: null });
        toast(`✓ ${d.name} resumed`);
        const freshState = await loadState();
        renderDevices(freshState);
        renderQuickDevices(freshState);
        renderQuickDevicesOverview(freshState);
      });
      tdPause.appendChild(resumeBtn);
    } else {
      const pauseWrapper = document.createElement('div');
      pauseWrapper.className = 'pause-controls';
      const pauseBtn = document.createElement('button');
      ...
      [15, 30, 60, 240, 1440].forEach(mins => {
        ...
        btn.addEventListener('click', async () => {
          const pauseUntil = new Date(Date.now() + mins * 60000).toISOString();
          await updateDevice(d.id, { pausedUntil: pauseUntil });
          toast(`✓ ${d.name} paused for ${fullLabel}`);
          pauseMenu.style.display = 'none';
          const freshState = await loadState();
          renderDevices(freshState);
          renderQuickDevices(freshState);
      ...
      pauseWrapper.append(pauseBtn, pauseMenu);
      tdPause.appendChild(pauseWrapper);
    }
